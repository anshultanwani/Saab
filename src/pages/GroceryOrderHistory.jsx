...
const GroceryOrderHistory = (props) => {
  const { deliveryCharges, highDemandCharges } = props;
  ...
  useEffect(() => {
    if (sessionStorage.getItem("isLoggedIn") === "true") {
      axios
        .get(window.apiDomain + "/v1/orders?userId=" + userId)
        .then((res) => {
          console.log(res);
          ...
          Object.keys(res.data.data).map((index) => {
            ...
            Object.keys(res.data.data[index].items).map((cur) => {
              res.data.data[index].items[cur].map((val) => {
                itemsArr.push(val.name);
              });
            });
            itemsArr.unshift(res.data.data[index].status);
                ...
                <div className="left">
                  <p>
                    <span>
                      <img
                        src={
                          require("../assets/images/grocey-icon1.png").default
                        }
                        alt="not loaded"
                      />
                    </span>
                    ...
                    <p className="list-items">
                      ...
                      <span>
                        <button type="button" onClick={() => setDp(!dp)}>
                          <img
                            src={
                              require("../assets/images/arrrow-up.png").default
                            }
                            alt="not loaded"
                          />
                        </button>
